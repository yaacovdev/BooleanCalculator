...
function separateIntoUniqueInputs(stackInp){
    for(var i = 0; i < stackInp.length; i++){
        ...
        for(var j = 0; j < varArray.length; j++){
            if(varArray[j][0] == stackInp[i].formula){
                isIn = true;
                break;
            }
        }
        if(stackInp[i].order.length != 0 || isIn) continue;
        varArray.push([stackInp[i].formula]);
    }
}

function convertToFormulaStack(Stack){
    var FormulaStack = [];
    for(var i = 0; i < Stack.length; i++){
        FormulaStack.push(Stack[i].formula);
    }
    return FormulaStack;
}
...
function createTable(stackInput){
    ...
    stackInput = convertToFormulaStack(stackInput);
